...
public class LOTRAlignmentValues
{
    ...
    public static AlignmentBonus MARRIAGE_BONUS;
    public static AlignmentBonus FANGORN_TREE_PENALTY;
    public static AlignmentBonus ROHAN_HORSE_PENALTY;
    public static AlignmentBonus VINEYARD_STEAL_PENALTY;
    private static final DecimalFormat alignFormat;
    private static final DecimalFormat conqFormat;
    private static final DecimalFormatSymbols alignFormatSymbols;
    
    public static AlignmentBonus createMiniquestBonus(final float alignment) {
        return new AlignmentBonus(alignment, "lotr.alignment.miniQuest");
    }
    
    public static AlignmentBonus createPledgePenalty(final float alignment) {
        return new AlignmentBonus(alignment, "lotr.alignment.breakPledge");
    }
    
    public static String formatAlignForDisplay(final float alignment) {
        return formatAlignForDisplay(alignment, LOTRAlignmentValues.alignFormat, true);
    }
    
    public static String formatConqForDisplay(final float conq, final boolean prefixPlus) {
        return formatAlignForDisplay(conq, LOTRAlignmentValues.conqFormat, prefixPlus);
    }
    
    private static String formatAlignForDisplay(final float alignment, final DecimalFormat dFormat, final boolean prefixPlus) {
        char decimalSeparatorChar = '.';
        char groupSeparatorChar = ',';
        final String decimalSeparator = StatCollector.translateToLocal("lotr.alignment.decimal_separator_char");
        if (decimalSeparator.length() == 1) {
            decimalSeparatorChar = decimalSeparator.charAt(0);
        }
        final String groupSeparator = StatCollector.translateToLocal("lotr.alignment.group_separator_char");
        if (groupSeparator.length() == 1) {
            groupSeparatorChar = groupSeparator.charAt(0);
        }
        LOTRAlignmentValues.alignFormatSymbols.setDecimalSeparator(decimalSeparatorChar);
        LOTRAlignmentValues.alignFormatSymbols.setGroupingSeparator(groupSeparatorChar);
        dFormat.setDecimalFormatSymbols(LOTRAlignmentValues.alignFormatSymbols);
    ...
    public static void notifyAlignmentNotHighEnough(final EntityPlayer entityplayer, final float alignmentRequired, final LOTRFaction faction) {
        final IChatComponent componentAlignReq = (IChatComponent)new ChatComponentText(formatAlignForDisplay(alignmentRequired));
    ...
    public static void notifyAlignmentNotHighEnough(final EntityPlayer entityplayer, final float alignmentRequired, final LOTRFaction faction1, final LOTRFaction faction2) {
        final IChatComponent componentAlignReq = (IChatComponent)new ChatComponentText(formatAlignForDisplay(alignmentRequired));
        componentAlignReq.getChatStyle().setColor(EnumChatFormatting.YELLOW);
    ...
    public static void notifyAlignmentNotHighEnough(final EntityPlayer entityplayer, final float alignmentRequired, final LOTRFaction faction1, final LOTRFaction faction2, final LOTRFaction faction3) {
        final IChatComponent componentAlignReq = (IChatComponent)new ChatComponentText(formatAlignForDisplay(alignmentRequired));
        componentAlignReq.getChatStyle().setColor(EnumChatFormatting.YELLOW);
    ...
    public static void notifyMiniQuestsNeeded(final EntityPlayer entityplayer, final LOTRFaction faction) {
        entityplayer.addChatMessage((IChatComponent)new ChatComponentTranslation("chat.lotr.requireMiniQuest", new Object[] { faction.factionName() }));
    }
    
    static {
        LOTRAlignmentValues.MARRIAGE_BONUS = new AlignmentBonus(5.0f, "lotr.alignment.marriage");
        LOTRAlignmentValues.FANGORN_TREE_PENALTY = new AlignmentBonus(-1.0f, "lotr.alignment.cutFangornTree");
        LOTRAlignmentValues.ROHAN_HORSE_PENALTY = new AlignmentBonus(-1.0f, "lotr.alignment.killRohanHorse");
        LOTRAlignmentValues.VINEYARD_STEAL_PENALTY = new AlignmentBonus(-1.0f, "lotr.alignment.vineyardSteal");
        alignFormat = new DecimalFormat(",##0.0");
        conqFormat = new DecimalFormat(",##0.00");
        alignFormatSymbols = new DecimalFormatSymbols();
    }
    ...
    public static class AlignmentBonus
    {
        ...
        public AlignmentBonus(final float f, final String s) {
            this.needsTranslation = true;
            this.isKill = false;
            this.killByHiredUnit = false;
            this.isCivilianKill = false;
            this.bonus = f;
            this.name = s;
        }
        
        public static float scalePenalty(float penalty, final float alignment) {

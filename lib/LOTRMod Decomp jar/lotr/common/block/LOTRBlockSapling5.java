...
public class LOTRBlockSapling5 extends LOTRBlockSaplingBase
{
    ...
    public void growTree(final World world, final int i, final int j, final int k, final Random random) {
        ...
        if (meta == 0) {
            treeGen = (WorldGenerator)LOTRTreeType.PINE.create(true, random);
        }
        if (meta == 1) {
            treeGen = (WorldGenerator)LOTRTreeType.LEMON.create(true, random);
        }
        if (meta == 2) {
            treeGen = (WorldGenerator)LOTRTreeType.ORANGE.create(true, random);
        }
        if (meta == 3) {
            treeGen = (WorldGenerator)LOTRTreeType.LIME.create(true, random);
        }

...
public class LOTRTeleporterElvenPortal extends Teleporter
{
    ...
    public LOTRTeleporterElvenPortal(final WorldServer world) {
        super(world);
                    ...
                    for (int j2 = this.theWorld.getActualHeight() - 1; j2 >= 0; --j2) {
                        ...
                        for (int i3 = i2 - 1; i3 <= i2 + 1; ++i3) {
                            for (int k3 = k2 - 1; k3 <= k2 + 1; ++k3) {
                                if (this.theWorld.getBlock(i3, j2, k3) != LOTRMod.elvenPortal) {
                                    portalHere = false;
                                }
                            }
                        }
    ...
    public boolean makePortal(final Entity entity) {
        ...
        for (int i4 = -2; i4 <= 2; ++i4) {
            for (int k4 = -2; k4 <= 2; ++k4) {
                for (int j2 = generateDirtBelow ? -1 : 0; j2 <= 2; ++j2) {
                    ...
                    else if (j2 < 0 && generateDirtBelow) {
                        this.theWorld.setBlock(i3, j5, k5, Blocks.dirt, 0, 2);
                    }
                    else {
                        final boolean isFrame = i4 == -2 || i4 == 2 || k4 == -2 || k4 == 2;
                        this.theWorld.setBlock(i3, j5, k5, isFrame ? LOTRMod.quenditeGrass : LOTRMod.elvenPortal, 0, 2);
                    }
                }
            }
        }
        for (int i4 = -3; i4 <= 3; ++i4) {
            for (int k4 = -3; k4 <= 3; ++k4) {
                for (int j2 = generateDirtBelow ? -2 : -1; j2 <= 3; ++j2) {
                    final int i3 = actualPosX + i4;
                    final int j5 = actualPosY + j2;
                    final int k5 = actualPosZ + k4;

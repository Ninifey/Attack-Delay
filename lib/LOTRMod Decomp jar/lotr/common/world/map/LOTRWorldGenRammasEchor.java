...
{
    public static LOTRWorldGenRammasEchor INSTANCE;
    private int centreX;
    private int centreZ;
    private int radius;
    ...
    private LOTRWorldGenRammasEchor(final boolean flag) {
        super(flag);
        this.radius = 500;
            ...
            for (int i2 = i; i2 <= i + 15; ++i2) {
                for (int k2 = k; k2 <= k + 15; ++k2) {
                    ...
                    if (circleDist < this.wallThick) {
                        ...
                        final boolean ladder = false;
                        for (int j2 = this.wallTop; j2 > 0; --j2) {
                            if (fences) {
                                this.setBlockAndMetadata(world, i2, j2, k2, Blocks.fence, 0);
                            }
                            else {
                                if (j2 >= this.wallTop && wallEdge) {
                                    this.setBlockAndMetadata(world, i2, j2, k2, LOTRMod.brick2, 11);
                                }
                                else if (j2 == this.wallTop && circleDist < 0.015) {
                                    this.setBlockAndMetadata(world, i2, j2, k2, (Block)Blocks.wooden_slab, 0);
                                }
                                else {
                                    this.setBlockAndMetadata(world, i2, j2, k2, LOTRMod.brick, 1);
                                }
                                if (wallEdge && j2 == this.wallTop && !ladder) {
                                    ...
                                    if (IntMath.mod(i2 + k2, 2) == 1) {
                                        ...
                                        this.setBlockAndMetadata(world, i2, j2 + 2, k2, Blocks.fence, 0);
                                        this.setBlockAndMetadata(world, i2, j2 + 3, k2, Blocks.torch, 5);
                                    }
                                }
                                if (ladder) {
    ...
    static {
        LOTRWorldGenRammasEchor.INSTANCE = new LOTRWorldGenRammasEchor(false);
    }
}
